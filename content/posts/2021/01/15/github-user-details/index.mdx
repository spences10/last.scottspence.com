...
This query will give the last 100 repos that aren't forks for that
GitHub username.

Here's what the response from the GraphQL query looks:
...
I then transform the data so it will go into the data shape the pie
chart is expecting. Check out the [data transform] module on the repo
for more detail and also Leigh's video [Map, Reduce, Filter, and Pie
Charts] is super helpful.
