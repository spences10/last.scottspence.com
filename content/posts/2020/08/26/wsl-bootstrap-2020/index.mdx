...
(yes) and `yarn add` to `ya`. this time I'll need to move the created
`.ssh` folder to
...
No to be able to use Git with GitHub and not having to authenticate
each time, to do that SSH!

I'll follow my own instructions I've made for myself in [Cheat Sheets
again], this time I'll need to move the contents of the created `.ssh`
folder to the location on my C drive for safe keeping.
...
[earlier]: #enable-file-permissions-for-symlinks this time I'll need
to move the created `.ssh` folder to [dotfiles repo]:
https://github.com/spences10/dotfiles/blob/master/Windows/.gitconfig
[cheat sheets again]: https://cheatsheets this time I'll need to move
the created `.ssh` folder to
.xyz/git/#how-to-authenticate-with-github-using-ssh
