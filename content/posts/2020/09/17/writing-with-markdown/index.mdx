...
so the next approach is to use hashes `#` for going from h1 (single
...
With this way of doing Markdown headings is if you put this in a
...
With Markdown it's the same as HTML with unordered and ordered lists
and like the differing flavours of headings and text decorations with
bold and italic there's several ways to make **unordered lists**:
...
```md
| Markdown       |  Tables   |         Layout |
| -------------- | :-------: | -------------: |
| Left aligned 1 | Centred 1 | Left aligned 1 |
| Left aligned 2 | Centred 2 | Left aligned 2 |
| Left aligned 3 | Centred 3 | Left aligned 3 |
```
