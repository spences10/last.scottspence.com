...
variable that the `PostBySlug` query (`PostBySlug($slug: String!)` {})
is expecting, that's where `slug` is defined with the `$` to indicate
...
Super duper! Now I can see that the Gatsby File System Route API is
doing it's magic
